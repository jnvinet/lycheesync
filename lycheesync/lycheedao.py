...
class LycheeDAO:
    ...
    def createAlbum(self, album):
        ...
        try:
            ...
            cur.execute("insert into lychee_albums (id, title, sysstamp, public, password, min_takestamp, max_takestamp) values (%s,%s,%s,%s,NULL,0,0)", (album['id'], album['name'], datetime.datetime.now().strftime('%s'), str(self.conf["publicAlbum"])))
            self.db.commit()
